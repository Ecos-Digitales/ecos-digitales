...
const ArticleResultCard = ({
  ...
  const formattedDate = format(new Date(result.date), "d MMM", {
    locale: es
  }).toUpperCase();
  return <Link to={`/noticias/${result.slug}`} className="group block">
      <article className="flex items-start gap-4 rounded-xl border border-border bg-background p-4 transition-all duration-200 hover:border-primary/20 hover:shadow-card-hover sm:gap-6 sm:p-5">
        ...
        {result.image && <div className="relative h-20 w-24 flex-shrink-0 overflow-hidden rounded-lg bg-muted">
            <img src={result.image} alt={result.title} className="h-full w-full object-cover transition-transform duration-[450ms] group-hover:scale-[1.048]" />
          </div>}
      </article>
    </Link>;
};
const JobResultCard = ({
  ...
  return <Link to={`/trabajos/${result.slug}`} className="group block">
      <article className="flex items-start gap-4 rounded-xl border border-border bg-background p-4 transition-all duration-200 hover:border-primary/20 hover:shadow-card-hover sm:gap-6 sm:p-5">
        <div className="flex flex-1 flex-col min-w-0">
          <h3 className="text-sm font-medium leading-snug text-foreground transition-colors group-hover:text-primary line-clamp-2 sm:text-base">
            {result.title}
          </h3>
          {result.company && <p className="text-xs text-muted-foreground mt-0.5">{result.company}</p>}
          <p className="text-xs text-muted-foreground line-clamp-2 mt-1">{result.description}</p>
          <span className="text-xs text-muted-foreground mt-auto pt-3">{relativeDate}</span>
        </div>
        {result.image && <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-muted">
            <img src={result.image} alt={result.company || result.title} className="h-full w-full object-contain p-2" />
          </div>}
      </article>
    </Link>;
};
const Search = () => {
  ...
  const seoDescription = hasQuery ? `Resultados de búsqueda para "${localQuery}" - Encuentra artículos y trabajos tech en Nucleo` : "Busca artículos de tecnología y oportunidades laborales en Nucleo";
  return <>
      <SEO title={seoTitle} description={seoDescription} url="https://serif-stream.lovable.app/buscar" type="website" />
      ...
      <main className="flex-1">
        <section className="container py-6">
          ...
          <div className="relative mb-6">
            ...
            <input type="text" placeholder="Buscar artículos y trabajos..." value={localQuery} onChange={e => setLocalQuery(e.target.value)} autoFocus className="h-12 w-full rounded-lg border border-border bg-background pl-12 pr-4 text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-all" />
          </div>

          {isLoading ? <LoadingGrid /> : !hasQuery ? <div className="py-8 text-center">
              <p className="text-muted-foreground">
                Escribe para buscar artículos y trabajos
              </p>
            </div> : <div className="space-y-8">
              {/* Matching Jobs */}
              {matchingJobs.length > 0 && <div>
                  <h2 className="text-lg font-semibold text-foreground mb-4">
                    Trabajos
                  </h2>
                  <div className="flex flex-col gap-3">
                    {matchingJobs.map(result => <JobResultCard key={`job-${result.id}`} result={result} />)}
                  </div>
                </div>}

              {/* Matching Articles */}
              {matchingArticles.length > 0 && <div>
                  <h2 className="text-lg font-semibold text-foreground mb-4">
                    Noticias
                  </h2>
                  <div className="flex flex-col gap-3">
                    {matchingArticles.map(result => <ArticleResultCard key={`article-${result.id}`} result={result} />)}
                  </div>
                </div>}

              {/* Recent Articles when no article matches */}
              {showRecentArticles && recentArticles.length > 0 && <div>
                  <h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                    
                    Últimas noticias
                  </h2>
                  <div className="flex flex-col gap-3">
                    {recentArticles.map(result => <ArticleResultCard key={`recent-${result.id}`} result={result} />)}
                  </div>
                </div>}

              {/* No results at all */}
              {matchingJobs.length === 0 && matchingArticles.length === 0 && !showRecentArticles && <div className="py-8 text-center">
                  <p className="text-muted-foreground">
